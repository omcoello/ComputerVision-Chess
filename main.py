...
def main():
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            #     start_click_time = time.time()  # Registrar el tiempo de inicio del clic
            # elif event.type == pygame.MOUSEBUTTONUP and event.button == 1:  # Soltar el botón izquierdo del mouse
            #     if start_click_time is not None:
            #         elapsed_time = time.time() - start_click_time
            #         if elapsed_time >= 2:
            #             pos = pygame.mouse.get_pos()
            #             row, col = get_row_col_from_mouse(pos)
            #             game.select(row, col)
            #     start_click_time = None
